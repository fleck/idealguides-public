...
const Indexers = ({
  ...
  useEffect(() => {
    if (hostnameFilter) {
      void debouncedPush({
        query: { hostname: hostnameFilter },
      })
    } else {
      void debouncedPush({ query: {} })
    }
  }, [hostnameFilter])
        ...
        <tbody>
          {indexers
            ...
            .map((indexer) => (
              <tr key={indexer.id}>
                <th>
                  <Link href={Routes.EditIndexer({ id: indexer.id })}>
                    <a>Edit</a>
                  </Link>
                </th>
...
Indexers.getLayout = (page: any) => <Layout title={"All Indexers"}>{page}</Layout>
