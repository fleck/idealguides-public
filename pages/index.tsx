...
import { validHeightAndWidth } from "app/items/components/Properties"
import ct from "class-types.macro"
import { urlFor } from "app/file/url"

import { prefetchFromEventTarget } from "../app/prefetchData"
import { getHomePageItems } from "app/items/queries/getHomePageItems"
import { getFromCache } from "app/items/queries/getFromCache"
import { putItemInCache } from "app/items/mutations/putItemInCache"
...
export const getServerSideProps = async (ctx: GetServerSidePropsContext) => {
  ...
  cache({ ...ctx, for: [365, "days"], items }).catch(console.warn)
...
const Home = ({ items }: ItemsResult) => {
  ...
  return (
    <Layout
      navItems={[
        { onClick: () => setEditMode((previousEditMode) => !previousEditMode), text: "Edit" },
      ]}
    >
      {editMode && (
        <>
      ...
      <div className="container">
        <main tabIndex={-1} className="focus:outline-none container px-3 mx-auto flex-grow">
          <div className="py-12">
                ...
                <dl className="space-y-10 md:space-y-0 md:grid md:grid-cols-2 md:gap-x-8 md:gap-y-10">
                  <div className="relative">
                    ...
                    <dd className="mt-2 ml-16 text-base text-gray-500">
                      If we have 10 guides that share common steps they can be shared.{" "}
                      <strong className="text-yellow-700">
                        Update a list on one guide, and it&apos;s updated on all the other guides
                        that include it.
                      </strong>
                    </dd>
                  </div>
                  <div className="relative">
                    ...
                    <dd className="mt-2 ml-16 text-base text-gray-500">
                      ...
                      <strong className="text-yellow-700">stop wasting time</strong> on discontinued
                      items outdated pricing.
                    </dd>
                  </div>
                  <div className="relative">
                    <dt>
                    ...
                    <dd className="mt-2 ml-16 text-base text-gray-500">
                      Don&apos;t see the comparison you&apos;re looking for? Go ahead and create it!
                      Information seem incorrect or out of date? You can correct it. IdealGuides
                      allows our users to help keep guides{" "}
                  ...
                  <div className="relative">
                    ...
                    <dd className="mt-2 ml-16 text-base text-gray-500">
                      How many years till that solar panel installation pays off? See the calculated
                      payoff time based on up to date utility rates and product pricing.
                    </dd>
                  </div>
                </dl>
              </div>
...
const firstImage = (item: ItemResult) =>
  item.properties.find((property) => property.featured && property.datum.image)?.datum.image
...
function ItemTile({ item }: { item: ItemResult }) {
  ...
  return (
    ...
    >
      <Link href={Routes.ShowItemPage({ itemUrl: item.url })}>
        <a
          className={ct(
            "h-full",
            "overflow-hidden",
