...
export default async function coverage(_req: NextApiRequest, res: NextApiResponse) {
  await purgeCdnCache()
...
const cloudflareCacheClearingUrl = `http${process.env.FAKE_CLOUDFLARE_DOMAIN ? "" : "s"}://${
...
function clearInChunks(allCachedUrls: CachedUrl[]) {
  return Promise.all(
    chunk(allCachedUrls, 30).map(async function clearThirtyUrlsPerFetch(cachedUrls) {
      const urlsToClear = cachedUrls.map(({ headers, url }) =>
        typeof headers === "object" && headers && "" in headers ? { url, headers } : url,
      )

      assert(process.env.CLOUDFLARE_ZONE, "CLOUDFLARE_ZONE environment variable is required")
      assert(process.env.CLOUDFLARE_KEY, "CLOUDFLARE_KEY environment variable is required")
      assert(process.env.CLOUDFLARE_EMAIL, "CLOUDFLARE_EMAIL environment variable is required")
