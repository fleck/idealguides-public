...
test("adding child item", async ({ page }) => {
   ...
   */
  await page.click('[aria-label="Add child to How to Trim Hedges"]', { delay: 300 })
  ...
  await page.type('[aria-label="Title for new item"]', "Cut them with a Trimmer")

  await Promise.all([
    page.click('[aria-label="Save Cut them with a Trimmer"]'),
    page.waitForResponse((resp) => resp.url().includes("createChild") && resp.status() === 200),
  ])

  await page.reload()

  expect(await page.waitForSelector(':text("Cut them with a Trimmer")')).toBeTruthy()
})
