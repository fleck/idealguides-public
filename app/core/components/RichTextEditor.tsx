...
import { XIcon, ArrowUpIcon, ArrowDownIcon, TrashIcon } from "@heroicons/react/outline"
...
import { UseMutationFunction } from "app/items/queries/TypeUtilities"
...
export const ExistingItemsContainer = ({ query = "" }) => {
    ...
    <>
      ...
      {Boolean(query) && (
        <Suspense fallback={<div>loading</div>}>
          <ExistingItemsContainer {...{ query, setQuery }} />
        </Suspense>
      )}
    </>
  )
      ...
      >
        ...
        <select
          ...
          onChange={({ target: { value } }) => {
            const itemTypeKeys = Object.keys(ItemType) as ReadonlyArray<ItemType>
        ...
        {child && (
          <select
            ...
            onChange={({ target: { value } }) => {
              const titleTypeKeys = Object.keys(TitleType) as ReadonlyArray<TitleType>

              if (titleTypeKeys.includes(value)) {
                setParentItem?.((prevItem) => ({
                  ...prevItem,
                  children: prevItem.children?.map((sibling) =>
                    sibling === child ? { ...sibling, title_type: value } : sibling,
                  ),
                }))
              } else {
        ...
        <div className="relative flex items-start py-4 ml-2">
          ...
          <div className="ml-2 flex items-center h-5">
            <input
              ...
              onChange={() =>
                setItem((prevItem) => ({ ...prevItem, standalone: !prevItem.standalone }))
              }
        ...
        {parentItem?.children?.[0] !== child && (
          <Button
            aria-label="Move Up"
            onClick={() => move({ position: -1, child, setParentItem, updateChildren })}
          >
            <ArrowUpIcon />
          </Button>
        )}
            ...
            >
              ...
              >
                ...
                <div className="sm:flex sm:items-start">
                  <div className={ct("mt-3", "w-full", "text-center", "sm:mt-0", "sm:text-left")}>
                      ...
                      <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                        ...
                        <div className={ct("mt-2", "text-sm", "text-gray-500")}>
                          ...
                          <p>
                            {parents?.length > 0 && (
                              <>
                                Deleting this item will affect the following item
                                {parents.length > 1 ? "s" : ""}:
                              </>
                            )}
                          </p>
...
function SaveIcon({ dirty }: { dirty: boolean }) {
  return (
    <svg
...
function move({
  ...
  setParentItem?.((prevParentItem) => {
    ...
    reorderedChildren.splice(prevParentItem.children.indexOf(child) + position, 0, child)
