...
import createProperty from "app/properties/mutations/createProperty"
import updateProperty from "app/properties/mutations/updateProperty"
...
import { UpdateDatum } from "app/properties/mutations/Datum"
...
export default function PropertyForm({
  ...
  const [createPropertyMutation, { error, isLoading }] = useMutation(createProperty)
  ...
  const { localURL, getRootProps, open, fileInput, getInputProps, filename, file, progress, done } =
    useFileUpload()
      ...
      <Form
        ...
        onSubmit={async (values, form, cb) => {
          ...
          if (isNewRecord(property)) {
            updatedProperty = await createPropertyMutation({
              ...values,
              itemId: item.id,
          ...
          await onSubmit?.({ ...values, fileId: file?.id?.toString() }, form, cb)
        }}
      >
          ...
          <div className="mt-1 sm:mt-0 sm:col-span-2" {...getRootProps()}>
            <div className="max-w-lg flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md">
              <div className="space-y-1 text-center">
                ...
                {!localURL && <p className="text-xs text-gray-500">PNG, JPG, GIF up to 10MB</p>}
                {filename}
              </div>
            </div>
          </div>
        ...
        {isLoading && <Loading />}
        {Boolean(error) && (
          <span className={ct("font-semibold", "text-red-700")}>{message(error)}</span>
        )}
        {Boolean(property.datum.indexerId) && user && <Socket {...{ user, property, setItem }} />}
        ...
        <FormSpy
          onChange={(props) => {
            ...
            const dirtyValues = Object.fromEntries(
              ...
              Object.entries({ [props.active]: "", ...props.values }).filter(([key]) =>
            ...
            const newPropertyValues = Object.fromEntries(
              Object.entries(dirtyValues).filter(([key]) => properties.includes(key)),
            )
...
const Socket = ({ user, setItem }: IndexerProps) => {
  const { lastJsonMessage }: { lastJsonMessage: { updatedData: Datum[] } | null } = useWebSocket(
