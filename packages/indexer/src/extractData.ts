...
import type { DataToIndex } from "app/properties/queries/getData"
import { replacementsSchema, selectorsSchema } from "./IndexerSchema"
import { parse } from "@babel/parser"
import { Indexer } from "db"
import get from "lodash/get"

const extractDataFromPage = async (page: Page, { indexer, id }: DataToIndex[number]) => {
  const parsedSelectors = selectorsSchema.safeParse(indexer?.selectors)

  if (!parsedSelectors.success) {
    return { id, value: "", error: `No selectors could be found for indexer ${indexer?.id}` }
  }
    ...
    replacements.data.forEach(({ search, replacement = "" }) => {
      try {
        ...
        if (
          ...
          value = value.replace(
            new RegExp(statement.expression.pattern, statement.expression.flags),
            replacement,
          )
        } else {
...
const extractDataFromString = async (
  ...
  try {
    if (indexer.indexType === "JSON") {
      const parsedSelectors = selectorsSchema.safeParse(indexer.selectors)
      if (!parsedSelectors.success) {
        return { ...datum, error: `No selectors could be found for indexer ${indexer.id}` }
      }
  ...
  Promise.all(
    dataToIndex.map(async (datumToIndex) => {
      ...
      if (datumToIndex.indexer?.nextIndexer) {
        refinedDatum = await extractDataFromString(refinedDatum, datumToIndex.indexer.nextIndexer)
